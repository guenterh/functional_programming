...
object GenericFunctionExercises {
  ...
  case class Predicate[A](eval: A => Boolean) {
    ...
    //ist das richtig? müsste es nichgt isEven
    def flip: Predicate[A] =
      Predicate[A](par => !eval(par))
...
//  val isCapitalized:Predicate[User] = Predicate(user => user.name.capitalize == user.name)
...
//      && user.name.length >= 3
//      //&& user.name.substring(0,0) == user.name.substring(0,0).toUpperCase )
//      && user.name.capitalize == user.name
    isAdult && isUserNameLongerThan3 && isCapitalized

    //by(_.age)(???: Predicate[Int]) &&
  ...
  new JsonDecoder[LocalDate] {
    override def decode(json: Json): LocalDate = {
      val dT = stringDecoder.decode(json)
      LocalDate.parse(dT,DateTimeFormatter.ISO_LOCAL_DATE)
    }
  }
